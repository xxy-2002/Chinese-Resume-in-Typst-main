...
#sidebar(with-line: false, side-width: 15%)[
  *编程语言*
  ...
  *算法与 AI*
  ...
  熟悉 *C++* (STL, 并发, C++11), *Go*, 了解 Python; 熟悉常用设计模式
  
  熟悉 *HTTP/TCP* 协议; 熟悉 *MySQL* (B+树, 锁, 事务), *Redis* (缓存设计, 分布式锁)
  
  熟悉 *Git*, *Docker*, *Xmake*, *Cmake*; 了解 *Linux* 常用命令

  熟悉常用数据结构与算法; 掌握大模型微调与部署;熟悉 *PyTorch* 及深度学习模型; 
]
...
- *业务背景*：参与 *Flow (Papago.ai)* 智能体开发，该平台通过*无代码架构*为 *Lofty*、TruckerPath 等业务线提供核心 *AI 引擎*与运行时支持
- *多模型集成与分布式记忆架构*：
  - 参与开发 *Model Adapter* 层适配 *Gemini* 与 *AWS Bedrock (Qwen)* 模型，支持*全量功能接入*；利用 *AsyncExitStack* 实现*连接池复用*，降低核心接口延迟 *20%*
  - 研发 *Cortex 记忆中枢*，支持单/多 Agent 间的*记忆共享、装载与逻辑隔离*；设计 *MemoryBuffer 异步缓冲策略*，通过*定时定量存入*机制优化存储语义密度，在降低存储成本的同时提升*长效记忆质量*；利用 *Redis + Lua* 脚本实现*分布式锁*，彻底解决高并发下的数据竞争问题
- *多智能体编排与治理*：
  - 针对 *Supervisor* 模式开发无状态 *Validator*，构建*“审计-反馈-修正”闭环*，显著提升复杂任务拆解时的*参数准确率*
  - 研发 *Agent Overlap Analyzer*，利用 *TF-IDF* 量化智能体职责重叠度以治理*意图漂移*，从源头解决多智能体*路由混淆*问题
  - 开发监督节点路由工具 *AgentSelector*，通过*强制 Tool Calling* 替代自然语言分类，极大增强了极端场景下的*路由稳定性*
- *工程效能与成本优化*：
  - 参与开发*端到端 LLM 自动化评估流水线*，支持 *Batch API*，基于 *Redis + MySQL* 实现了任务的*断点续传 (Checkpoint)* 机制，使大规模回归测试的计算成本降低 *50%*
  - 落地 *TOON 序列化协议*与*语义压缩技术*，在保持语义完整性的前提下，降低线上全链路 *Token 消耗*
- *Lofty Agent Studio 平台开发*：
  - 主导研发后端服务，开发登录接口基于 *AWS Cognito 与 Lambda* 实现 Lofty 生态系统的免密 *SSO 鉴权*与*Team 隔离*
  - *架构演进与能力开放*：设计并开发新的 *Workspace 级别通用接口*；该接口成功支撑 *NLP 团队*将“用户自定义智能体”动态接入现有业务流
== #fa-code 项目经历
